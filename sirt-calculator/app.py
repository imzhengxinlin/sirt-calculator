...
def predict_A(sess, scaler_params, features_f, threshold, feature_values):
    """Model A: LR ONNX"""
    missing = [k for k in features_f if k not in feature_values]
    if missing:
        raise KeyError(f"Missing features: {missing}")
    x = np.array([[feature_values[k] for k in features_f]], dtype=np.float32)
    x_scaled = scale_X(scaler_params, x).astype(np.float32)
    inp_name = sess.get_inputs()[0].name
    output_names = [o.name for o in sess.get_outputs()]
    prob_out = next((n for n in output_names if "prob" in n.lower()), output_names[-1])
    raw = sess.run([prob_out], {inp_name: x_scaled})[0]
...
def predict_batch_A(sess, scaler_params, features_f, threshold, df):
    """Model A batch: LR ONNX"""
    missing = [k for k in features_f if k not in df.columns]
    if missing: return None, missing
    X = scale_X(scaler_params, df[features_f].values.astype(float)).astype(np.float32)
    inp_name = sess.get_inputs()[0].name
    output_names = [o.name for o in sess.get_outputs()]
    prob_out = next((n for n in output_names if "prob" in n.lower()), output_names[-1])
    raw = sess.run([prob_out], {inp_name: X})[0]
    if isinstance(raw[0], dict):
        probs = np.array([r[1] for r in raw], dtype=float)
    elif hasattr(raw, "ndim") and raw.ndim == 2:
        probs = raw[:, 1]
    else:
