...
def load_models():
    ...
    for model_id in ["A", "F", "G"]:
        try:
            with open(model_dir / f"model_{model_id}.json", "r") as f:
                data = json.load(f)
            models[model_id] = {
                "data":  data,
                "error": None
            ...
            models[model_id] = {"data": None, "error": str(e)}
    return models

def predict_model(model_data, feature_values, threshold):
    if model_data["data"] is None:
    ...
    if data["type"] == "LR":
        # LR: 单一scaler，用原始DVH值
        X     = np.array([feature_values[k] for k in features], dtype=np.float64)
        mean  = np.array(data["scaler_mean"])
        scale = np.array(data["scaler_scale"])
        X_sc  = (X - mean) / scale
        coef      = np.array(data["coef"])
        intercept = data["intercept"]
        logit     = float(np.dot(X_sc, coef) + intercept)
        prob      = 1.0 / (1.0 + np.exp(-logit))

    elif data["type"] == "RF":
        # RF: 分组scaler
        dvh_feats   = data.get("dvh_features",   [])
        radio_feats = data.get("radio_features",  [])
        clin_feats  = data.get("clin_features",   [])

        # 分组标准化
        X_sc_parts = {}

        if dvh_feats:
            X_dvh  = np.array([feature_values[k] for k in dvh_feats])
            m_dvh  = np.array(data["scaler_dvh_mean"])
            s_dvh  = np.array(data["scaler_dvh_scale"])
            X_sc_parts.update({k: (feature_values[k]-m)/(s+1e-8)
                               for k,m,s in zip(dvh_feats, m_dvh, s_dvh)})

        if radio_feats and data.get("scaler_radio_mean"):
            X_r   = np.array([feature_values[k] for k in radio_feats])
            m_r   = np.array(data["scaler_radio_mean"])
            s_r   = np.array(data["scaler_radio_scale"])
            X_sc_parts.update({k: (feature_values[k]-m)/(s+1e-8)
                               for k,m,s in zip(radio_feats, m_r, s_r)})

        if clin_feats and data.get("scaler_clin_mean"):
            m_c = np.array(data["scaler_clin_mean"])
            s_c = np.array(data["scaler_clin_scale"])
            X_sc_parts.update({k: (feature_values[k]-m)/(s+1e-8)
                               for k,m,s in zip(clin_feats, m_c, s_c)})

        # 按features顺序排列
        X_sc_1d = np.array([X_sc_parts[k] for k in features])
        ...
        for tree in data["trees"]:
            ...
            node = 0
            while cl[node] != -1:
                if X_sc_1d[ft[node]] <= thr[node]:
                    node = cl[node]
                else:
                    node = cr[node]
            node_val = np.array(val[node][0])
            proba_sum += node_val / node_val.sum()
        prob = float(proba_sum[1] / len(data["trees"]))
    else:
        return None, None, f"Unknown model type: {data['type']}"
...
def main():
    ...
    if st.button("Predict response", type="primary", use_container_width=True):
        ...
        if err:
            ...
            st.info(
                f"Model {model_id} | Algorithm: {models[model_id]['data']['type'] if models[model_id]['data'] else 'N/A'} | "
