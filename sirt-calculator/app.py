...
| Algorithm | XGBoost (Model A: DVH only) |
| Training set | 134 lesions / 70 patients |
| Validation set | 78 lesions / 31 patients |
| Validation AUC | 0.895 (95% CI: reported in manuscript) |
| Threshold | Youden index on training set = 0.34 |
| Outcome | 4-month mRECIST (CR+PR = responder) |
| Features | 7 DVH parameters (no radiomics required) |
| Dosimetry | Y-90 PET/CT voxel-based DVH (MIM Software v7.3.4) |
| DeLong vs Model F | p = 0.953 (no significant difference) |
""")
        st.markdown("""
#### Feature extraction requirements
- **Dosimetry**: Post-treatment Y-90 PET/CT within 3 days, voxel-based DVH
- **DVH parameters**: Dmean, Dmin, SD, D10, V100, V120, nCI
""")
    else:
        st.markdown("""
#### Model information
...
| Threshold | Youden index on training set = 0.52 |
| Outcome | 4-month mRECIST (CR+PR = responder) |
| Radiomics | PyRadiomics v3.1.0, IBSI-compliant |
| Dosimetry | Y-90 PET/CT voxel-based DVH (MIM Software v7.3.4) |
| DeLong vs Model A | p = 0.953 (no significant difference) |
""")
        st.markdown("""
#### Feature extraction requirements
